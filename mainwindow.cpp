...
// Chemin vers la base d'images d'apprentissage
static const QString CHEMIN_IMAGES = "/home/cytech/ING1/Projet_Reconnaissance_Faciale/Image";
    ...
    : QDialog(parent)
{
    ...
    btnAdd->setStyleSheet("background:#28a745; color:white; padding:6px 18px; border-radius:6px; border:none;");
    btnCancel->setStyleSheet("background:#e0e0e0; color:#111; padding:6px 18px; border-radius:6px;");
...
void AddImageWindow::validateAdd() {
    ...
    // 2. Compter les images existantes pour nommer la nouvelle
    QDir dir(dossier);
    ...
    // 3. Copier et convertir via OpenCV (JPG → PNG)
    ...
    // 4. Insertion en BDD MySQL via DatabaseManager
    try {
        DatabaseManager db;
        ...
        // La copie fichier a réussi mais la BDD a échoué
      ...
      pipelineReady_(false)
{
    ...
    // ── Zone centrale : image choisie + photo de référence ───────
    mainLayout->addSpacing(10);

    QHBoxLayout *imagesLayout = new QHBoxLayout();
    imagesLayout->setSpacing(20);

    // Zone upload / aperçu image test
    ...
    connect(initThread, &QThread::started, this, [this, initThread]() {
        try {
            pipeline_ = new RecognitionPipeline(
                CHEMIN_IMAGES.toStdString(), 128, 0.0 /* seuil auto */);
...
void MainWindow::chooseImage() {
    ...
    QPixmap scaled = pix.scaled(380, 260, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    imagePreview->setPixmap(scaled);
    imagePreview->setVisible(true);
    uploadBox->setVisible(false);

    refImageLabel->setText("—");
    refImageLabel->setPixmap(QPixmap());
    resultLabel->setText("");
    btnReset->setVisible(false);
...
void MainWindow::onResultReady(ResultatReconnaissance res) {
    ...
    if (res.reconnu) {
        texte = QString(
            ...
            "Distance : %2  |  Seuil (quantile 95%%) : %3<br>"
            "Confiance : <b>%4%</b>"
        )
        .arg(QString::fromStdString(res.nom))
        .arg(res.distance,        0, 'f', 1)
        .arg(res.seuilQuantile95, 0, 'f', 1)
        .arg(res.confiance,       0, 'f', 1);
        ...
        texte = QString(
            ...
            "Distance : %1  |  Seuil (quantile 95%%) : %2<br>"
            "Confiance : <b>%3%</b>"
        )
        .arg(res.distance,        0, 'f', 1)
        .arg(res.seuilQuantile95, 0, 'f', 1)
